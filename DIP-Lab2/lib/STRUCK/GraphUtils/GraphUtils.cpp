...
// Graphing functions
//------------------------------------------------------------------------------
const CvScalar BLACK = cvScalar(CV_RGB(0,0,0));
const CvScalar WHITE = cvScalar(CV_RGB(255,255,255));
const CvScalar GREY = cvScalar(CV_RGB(150,150,150));
...
CvScalar getGraphColor(void)
{
	...
	countGraph++;
	switch (countGraph) {
	case 1:	return cvScalar(CV_RGB(60,60,255));	// light-blue
	case 2:	return cvScalar(CV_RGB(60,255,60));	// light-green
	case 3:	return cvScalar(CV_RGB(255,60,40));	// light-red
	case 4:	return cvScalar(CV_RGB(0,210,210));	// blue-green
	case 5:	return cvScalar(CV_RGB(180,210,0));	// red-green
	case 6:	return cvScalar(CV_RGB(210,0,180));	// red-blue
	case 7:	return cvScalar(CV_RGB(0,0,185));		// dark-blue
	case 8:	return cvScalar(CV_RGB(0,185,0));		// dark-green
	case 9:	return cvScalar(CV_RGB(185,0,0));		// dark-red
	default:
		countGraph = 0;	// start rotating through colors again.
		return cvScalar(CV_RGB(200,200,200));	// grey
	}
}
...
{
	customGraphColor = cvScalar(CV_RGB(R, G, B));
...
IplImage* drawFloatGraph(const float *arraySrc, int nArrayLength, IplImage *imageDst, float minV, float maxV, int width, int height, char *graphLabel, bool showScale)
{
	...
	if (graphLabel != NULL && strlen(graphLabel) > 0) {
		...
		cvPutText(imageGraph, graphLabel, cvPoint(30, 10), &font, cvScalar(CV_RGB(0,0,0)));	// black text
	}
